...
for m in sorted(months):
    ...
    lines.append('')
    lines.append('| 日期 | 散瓶 | 原箱 | 价差 | 指导价 | 信号 | 来源 |')
    lines.append('|------|------|------|------|--------|:--:|------|')
    ...
    for p in months[m]:
        ...
        sp = fmt(p['sanping'])
        yx = fmt(p['yuanxiang'])
        diff = spread(p['sanping'] if isinstance(p['sanping'], (int, float)) else None,
                      p['yuanxiang'] if isinstance(p['yuanxiang'], (int, float)) else None)
        gp_f = str(p['guide_price'])
        sig = p['signal'] or ''
        src = p['source'][:15]
        
        # 配对日标星
        if isinstance(p['sanping'], (int, float)) and isinstance(p['yuanxiang'], (int, float)):
            d = f'★ {d}'
        
        lines.append(f'| {d} | {sp} | {yx} | {diff} | ¥{gp_f} | {sig} | {src} |')
...
lines.append('')
lines.append('| 日期 | 散瓶 | 原箱 | 价差 | 指导价 | 信号 | 来源 | 备注 |')
lines.append('|------|------|------|------|--------|:--:|------|------|')
...
for p in data['prices']:
    ...
    lines.append(f'| {d} | {sp} | {yx} | {diff} | ¥{gp_f} | {sig} | {src} | {note} |')
